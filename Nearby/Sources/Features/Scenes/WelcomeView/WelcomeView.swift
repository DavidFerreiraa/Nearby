...
class WelcomeView: UIView {
    ...
    private func setupTips() {
        guard let icon1 = UIImage(named: "mapIcon")?.withRenderingMode(.alwaysOriginal) else { return }
        ...
        let tip2 = TipsView(icon: UIImage(named: "qrcode")?.withRenderingMode(.alwaysOriginal) ?? UIImage(),
            title: "Ative o cupom com QR Code",
            description: "Escaneie o código no estabelecimento para usar o benefício")
        
        let tip3 = TipsView(icon: UIImage(named: "ticket")?.withRenderingMode(.alwaysOriginal) ?? UIImage(),
